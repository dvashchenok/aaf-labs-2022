...
class Table:
    ...
    def Insert(self, varsToInsert):
        ...
        for column in self.columns:
            if self.columns[column][INDEX]:
                self.columns[column][INDEX].insert(
                    varsToInsert[self.columns[column][ID]], id)
    ...
    def CheckIfInColumns(self, columns):
        allCorrectColumns = len(
            [column for column in columns if column.text in self.columns])
        if len(columns) != allCorrectColumns:
            raise Exception(
                'There is no such columns as',
